    ...
    initialize: {
      value: function(board, opts) {
        ...
        var readCycle = function() {
          ...
          // cycle 0: humidity
          // cycle 1: temperature
          var isTemperatureCycle = cycle === 1;
          ...
          io.i2cReadOnce(address, register, READLENGTH, function(data) {
            if (isTemperatureCycle) {
              computed.temperature = getRaw(data);
            } else {
              // HUMIDITY
              computed.humidity = getRaw(data);
            }
  ...
  MPL3115A2: {
    ...
    initialize: {
      value: function(board, opts) {
        ...
        var toggleOneShot = function() {};

        io.i2cConfig(opts);

        io.i2cRead(address, this.REGISTER.PRESSURE, READLENGTH, function(data) {
          var msb = data[0];
          var csb = data[1];
          var lsb = data[2];
          var pressure_decimal;

          console.log("Pressure Read: ", msb.toString(16), csb.toString(16), lsb.toString(16))

          var pressure_whole = msb<<16 | csb<<8 | lsb;
          pressure_whole >>= 6; //Pressure is an 18 bit number with 2 bits of decimal. Get rid of decimal portion.

          lsb &= 0x30; //Bits 5/4 represent the fractional component
          lsb >>= 4; //Get it right aligned
          pressure_decimal = lsb/4.0; //Turn it into fraction
          
          computed.pressure = pressure_whole + pressure_decimal;
