    ...
    initialize: {
	  ...
	  board.io.sendOneWireSearch(pin, function(err, devices) {
		...
		board.io.sendOneWireReset(pin);
            ...
            var count = 0;
            var isConversionAvailable = function(done) {
              board.io.sendOneWireReset(pin);
		board.io.sendOneWireWriteAndRead(pin, devicesToRead[0], CONSTANTS.READ_SCRATCHPAD_COMMAND, CONSTANTS.READ_COUNT, function(err, data) {
		  ...
		  if(data[0]) {
		      ...
		      this.emit("error",err);
                      ...
                      //allows the function to manage receiving no data, by resetting about 4 counts. This seems to occur periodically. 
		      count++;
